...
def move_to(
    ...
    individuals = (
        ...
        # M3: we record each individual's position at the start of the day (i.e. before any movement steps this
        # day) as its daily activity centre. All steps taken during the day are then weighted by their distance from
        # this centre, so movement stays concentrated around a "home range" for the day instead of drifting
        # arbitrarily far.
        .with_columns(
            center_x=pl.col("x"),
            center_y=pl.col("y")
    ...
    for i in range(max_distance):
        ...
        if movement_finished is not None:
            individuals_after_movement.append(movement_finished.drop("step_done", "distance", "center_x", "center_y"))
...
print(move_to(bembidions, movement_map, 14, .8, HOME_RANGE_SIGMA))

# And we can simulate an entire year of movement.
for day in range(365):
    print(f"*** DAY {day + 1} ***")
    bembidions = move_to(bembidions, movement_map, 14, .8, HOME_RANGE_SIGMA)
...
for day in range(365):
    print(f"*** DAY {day + 1} ***")
    bembidions2 = move_to(bembidions2, movement_map2, 14, .8, HOME_RANGE_SIGMA)
...
for day in range(365):
    print(f"*** DAY {day + 1} ***")
    bembidions3 = move_to(bembidions3, movement_map2, 14, .8, HOME_RANGE_SIGMA)
